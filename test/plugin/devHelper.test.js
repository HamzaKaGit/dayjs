...
it('Warning Passing Unix timestamp as a String not Number', () => {
  const consoleSpy = jest.spyOn(console, 'warn')
  dayjs('1231231231231')
  expect(consoleSpy).toHaveBeenCalledWith('To parse a Unix timestamp like 1231231231231, you should pass it as a Number. https://dayjs.io/docs/en/parse/unix-timestamp-milliseconds')
})

it('Warning Enable customParseFormat plugin while passing the second format parameter', () => {
  const consoleSpy = jest.spyOn(console, 'warn')
  dayjs('2020', 'YYYY')
  expect(consoleSpy).toHaveBeenCalledWith('To parse a date-time string like 2020 using the given format, you should enable customParseFormat plugin first. https://dayjs.io/docs/en/parse/string-format')
})

it('Warning: Setting locale before loading locale', () => {
  const consoleSpy = jest.spyOn(console, 'warn')
  dayjs.locale('zh-cn')
  expect(consoleSpy).toHaveBeenCalledWith('Guessing you may want to use locale zh-cn, you have to load it before using it. https://dayjs.io/docs/en/i18n/loading-into-nodejs')
})

describe('dev-helper: diff() usage warnings', () => {
  const diffWarningMsg = 'Invalid usage: diff() requires a valid comparison date as the first argument. https://dayjs.io/docs/en/display/difference'
